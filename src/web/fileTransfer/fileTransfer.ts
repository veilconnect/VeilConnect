 ...
 *
 * 二进制帧 vs 早先的 base64 文本帧：免去 +33% 线上膨胀、每帧载荷近翻倍、消息条数减半、
 * 收发免 base64 编解码（更省 CPU/内存）。注意：经 TURN 中继时实测吞吐主要受【中继链路】
 * 制约且波动较大（实测 10MB 约 67–125s / ~80–150KB/s，与分帧方式关系不大）——二进制帧是
 * 正确性与开销上的改进,并非吞吐的决定因素。
 */
