...
- **多语言界面**：内置 **15 种语言**（zh-CN / zh-TW / en / ja / ko / es / fr / de / ru / ar / pt / it / hi / th / vi），可热切换；阿拉伯语等 RTL 语言自动镜像版面。门禁页、文件下载页同样本地化。默认语言按浏览器语言自动选择；匹配不上时托管版用 Cloudflare 边缘国家码兜底（不调第三方、不额外暴露 IP；自部署版回退默认）
...
- **加固信令服务器**：Origin 白名单、房间 token（HMAC 摘要存储）、连接限速 / 防 token 爆破（按 **HMAC(IP) 指纹**计数，不存明文 IP）、消息体积上限、CSP 响应头
- **可持久房间**：房间可选「空置不销毁、可重连」；注册表只存 roomId→token 摘要 / 容量 / 生命周期，**不存消息、IP、clientId**。自部署落盘、托管版存 Cloudflare Durable Object
- **匿名使用计数**：仅在「配对/SAS 验证成功」时由一端发一个**空信标**，服务器只对一个全局总数 + 按天总数 +1，**不含任何身份 / 房间 / IP / 内容 / 精确时间**。读取需 Bearer token（自部署者不部署该端点即不计数）

> **注**：当前一对一聊天为主线功能。信令层房间容量上限为 4，但开源网页客户端目前仍是 **1:1**（单棘轮会话），多方/团队房属未启用的商业层能力（见下）。

### 商业层（默认关闭，未启用）

仓库内含一套 `src/commercial/` 商业化脚手架（分层套餐、品牌定制、配额/计费、自部署许可、白标客户会话、团队房、应急频道、企业审计包）及配套 Cloudflare 控制面 Worker。**这层默认通过构建开关关闭、未接入运行时**（仅租户品牌展示一小片可选启用），是经单测验证但**尚未上线**的骨架，不影响开源自部署的纯隐私聊天使用。详见 [`docs/commercial/FOUNDATION.md`](docs/commercial/FOUNDATION.md)。
...
│   │   └── i18n/                      # 15 语言翻译 + RTL + Context Provider
│   └── commercial/                    # 商业层脚手架（默认关闭，未接入运行时）
│       ├── plans.ts·license.ts·billing.ts·tenant.ts·usage.ts # 套餐/许可/计费/租户/配额
│       ├── teamRoom.ts·clientSession.ts·emergency.ts·auditPack.ts # 团队房/白标/应急/审计
│       └── client.ts·featureFlags.ts  # 前端品牌加载器 + 构建开关
...
├── tests/                             # Jest 单元/集成测试（336 cases，含商业层 31）
└── jest.config.js
```
...
> `npx jest server/signaling-server.test.js --roots ./server --testMatch "**/*.test.js"` → 15 passed。
