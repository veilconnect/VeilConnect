...
脚本统一会：装 Docker（缺则装）+ 确保 docker compose v2 → 选定模式/域名 → 探测 IP → 生成 TURN 密钥 → 写 `.env` → 放行 ufw → `docker compose up -d --build`。重复运行幂等（复用已有地址、端口、TURN 密钥和传输设置）。
可用环境变量覆盖：`EXTERNAL_IP=` `BIND_IP=` `TURN_SECRET=` `HTTP_PORT=` `HTTPS_PORT=` `TURN_TRANSPORT=`。wg/内网部署可把 `EXTERNAL_IP` 与 `BIND_IP` 设为 wg 地址；脚本 local 模式默认只让 Caddy 监听选定地址。
