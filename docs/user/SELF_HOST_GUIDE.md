...
若你已手动 `git clone` 仓库，则在仓库目录内直接运行下面的 `install.sh`（三选一同理）：
...
脚本统一会：装 Docker（缺则装）+ 确保 docker compose v2 → 选定模式/域名 → 探测 IP → 生成 TURN 密钥、IP 指纹密钥、房间 token 摘要密钥和统计读取令牌 → 写 `.env` → 放行 ufw → `docker compose up -d --build`。重复运行幂等（复用已有地址、端口、TURN 密钥、信令隐私密钥、房间摘要密钥和传输设置）。
可用环境变量覆盖：`EXTERNAL_IP=` `BIND_IP=` `TURN_SECRET=` `SIGNAL_IP_HASH_SECRET=` `ROOM_TOKEN_HASH_SECRET=` `METRICS_READ_TOKEN=` `HTTP_PORT=` `HTTPS_PORT=` `TURN_TRANSPORT=`。wg/内网部署可把 `EXTERNAL_IP` 与 `BIND_IP` 设为 wg 地址；脚本 local 模式默认只让 Caddy 监听选定地址。
...
- **有域名（推荐）**：用 `install.sh <域名>` 部署，Caddy 自动签发 **Let's Encrypt** 证书 → **任何设备零配置、无警告**（公网服务器需 80/443 可达；纯 wg/内网 + 域名可用 DNS-01，需 DNS API）。
- **无域名（`--local` 自签）**：浏览器首访提示「不安全 / ERR_CERT_AUTHORITY_INVALID」属正常——证书不被系统预信任，但连接仍加密，且应用的端到端加密/防 MITM 靠**安全码核对**，与此证书无关。处理方式：
  1. 直接点「高级 → 继续前往」即可使用（wg/内网下足够安全）；
  2. 想消除警告：在设备上访问 **`https://<地址>/root-ca.crt`** 下载根证书，安装到「受信任的根证书颁发机构」（每台设备装一次，之后永久无警告）。
- **网页无法静默自动安装根证书**（浏览器/系统安全限制，防止任意网站 MITM）；要"零客户端操作"只能走域名 + 公网信任证书。
